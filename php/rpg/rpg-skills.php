	...
	<div class="d-inline-flex flex-12 flex-gap-48 flex-wrap flex-md-nowrap">
		<div class="d-inline-flex flex-6">
			<div class="game-mechanics">
				...
				<p class="copy">
					...
					<br><br>
					Skill checks will typically have one or more associated Outer Ring stat values. Add the associated stat total to the roll for the check. Some effects will require established stats outlined by the rules associated with whatever is requiring the check, but in most situations the GM will determine what stats are being measured. When a skill check asks for an Inner Ring stat, the character can decide which of the Outer Ring stats in that category they wish to use.
					<br><br>
					Skill checks are a way for a Game Master (GM) to measure a character against some form of adversity or to grade their performance. A skill check may have a target value that is known to the player or one that is known only to the GM. Some skill checks may not have a target value at all and are instead looking to see how "good" the roll is and rationalizing how that character's effort meets their need (such as establishing the quality of a character's work of art). How skills are truly utilized is typically a style of GMing more than a hard and fast set of rules.
					<br><br>
				...
				<ul class="copy">
					...
					<li>Some complex skill checks will reference more than one Outer Ring stat. For these checks the player needs to roll for each stat separately. The result is a combination of those successes and failures, and that result is up to the GM. Ex. A spy driving a car in a foreign land could have a skill check against both their Grace and Memory. The first for the act of driving itself, the second to demonstrate that they remember the road laws of the country they are infiltrating.</li>
					<li>Skill success is a gradient; succeeding by a small amount may technically succeed but may incur some unforeseen event. Meanwhile, failing or succeeding hugely may result in significant benefits or consequences. The extent to which a character succeeds or fails is up to the GM.
						<ul>
					...
					<li>Some skill checks can be repeatedly attempted until the character succeeds or stops trying. It is up to the GM to decide what checks can be repeated and how long they take.</li>
					<li>If a character's modifiers to the roll would mean that roll automatically meets or exceeds the check's target value, the player can forgo rolling at all if they wish, though the GM may still ask for them to roll in order to know how far they exceed the requirement.</li>
				</ul>
				...
				<ul class="copy">
					...
					<li>The GM can allow or deny requested advantage and will determine the appropriate expression of the advantage.</li>
					<li>Ex. A character with a background as a fisherman may be able to request advantage in a situation where they need to maintain balance while the airship they are on is suffering severe turbulence due to the character's past with the rocky seas. The GM could choose to give them a +5 to their Grace roll to stay standing.</li>
					<li>Ex. A character is a pilot and is piloting the ship they built by hand and know by heart. The GM could choose to give them a bonus to a skill check or even waive the need for the skill check if the act could be considered routine.</li>
				</ul>
				...
				<ul class="copy">
					...
					<li>A bard skilled with a guitar and a bass can likely request competence with a lute, but the GM may consider the violin to be a step too far.</li>
		...
		<div class="d-inline-flex flex-6">
			<div class="game-mechanics">
				...
				<p class="copy">
					Skill check requirements range from -10 to 80, with an explanation of what those values represent below. Higher checks require higher relevant rolls and modifiers to succeed. A check that notes a stat can have that stat added to its total. A Complex check that notes multiple stats must roll separately for each stat. In general, as actions grow more complex or multi-staged, the relevant skill checks will require more stats to be rolled against at once.
					<br><br>
